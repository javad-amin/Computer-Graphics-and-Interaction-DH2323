...
void Interpolate(Pixel a, Pixel b, vector<Pixel> &result) {
	...
	a.pos3d *= a.zinv;
	b.pos3d *= b.zinv;
	vec3 Step3d = vec3(b.pos3d - a.pos3d) / float(glm::max(N - 1, 1));
	...
	for (int i = 0; i < N; ++i)
	{
		...
		current.pos3d = ((Step3d * (float) i + a.pos3d) / current.zinv);
		result[i] = current;
	}
}
	...
	for (int i = 0; i<V; ++i)
	{
		...
		for (int j = 0; j<line.size(); ++j )
		{
			...
			if (p.x < leftPixels[rowIndex].x)
			{
				...
				leftPixels[rowIndex].pos3d = p.pos3d;
			}
			if (p.x >= rightPixels[rowIndex].x)
			{
				...
				rightPixels[rowIndex].pos3d = p.pos3d;
			}
		}
...
void PixelShader( const Pixel& p )
{
	...
	if( p.zinv > depthBuffer[y][x] )
	{
		...
		vec3 D = (lightPower * max(glm::dot(rhat, nhat), 0.0f)) / float(4.0f * 3.14f * r * r);
		// Total reflected light
		vec3 currentLight  = currentReflectance * (D + indirectLightPowerPerArea);
		PutPixelSDL( screen, x, y, currentColor * currentLight );
	}
}
